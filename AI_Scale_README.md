...
- access purchased videos anytime from their dashboard after payment
...
- **Video storage & delivery:** AWS S3 Bucket
...
5. Purchased videos are available anytime from the dashboard
6. Videos are stored in AWS S3 and access is controlled server-side
...
| multer | File uploads |
| aws-sdk | S3 operations if needed |
...
Purchased videos are permanently available after payment.

All videos are stored inside **AWS S3 Bucket**.

### How access works

1. User logs in
2. User purchases video
3. Payment webhook confirms payment
4. Video ID is added to `purchasedVideos`
5. User opens dashboard
6. Backend checks purchase status
7. Authorized users get access to the purchased video list and its S3-stored content

### Rules

- public listing shows metadata only
- videos remain locked until purchase
- purchased videos stay permanently accessible
- access verification happens server-side
...
NEXT_PUBLIC_S3_BUCKET=aiscale-videos
```
...
- S3-backed video storage and delivery
...
| Video delivery | ~2,000 concurrent | AWS S3 + CDN caching |
...
### Phase 6 — Video access
- S3 video storage
- purchase flow
- purchased video access control
- dashboard access to purchased content
...
- safe slot booking
- purchased video access via AWS S3
